...
namespace Lite.BevTreeEditor
{
	public class BTGraphNodeStyle
	{
		...
		private const float HORZ_MIN_NODE_HEIGHT = 80;
		private const float HORZ_NODE_WIDTH = 180;
		private const float NODE_BORDER = 20;


		// force horizontal
		private const float MAX_NODE_HEIGHT = 60;
		private const float MAX_NODE_WIDTH = 100;
		...
		public Vector2 GetSize_Horz(string content)
		{
			m_content.text = content;
			Vector2 size = BTEditorStyle.NodeTitleLabel.CalcSize(m_content);
			size.x = Mathf.Max(size.x + 10, MAX_NODE_WIDTH);
			size.y = Mathf.Max(size.y, MAX_NODE_HEIGHT);
