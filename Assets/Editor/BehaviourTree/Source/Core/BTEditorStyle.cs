...
namespace Lite.BevTreeEditor
{
	public static class BTEditorStyle
	{
		...
		public static BTEditorTreeLayout TreeLayout
		{
			get
			{
				return BTEditorTreeLayout.Horizontal;// (BTEditorTreeLayout)EditorPrefs.GetInt("Lite.BevTree.Editor.TreeLayout", (int)BTEditorTreeLayout.Vertical);
			}
			set
			{
				EditorPrefs.SetInt("Lite.BevTree.Editor.TreeLayout", (int)value);
			}
		}
		...
		public static BTGraphNodeStyle GetNodeStyle(BehaviourNode node)
		{
			if(node != null)
			{
				...
				else if(node is Lite.BevTree.Action)
				{
