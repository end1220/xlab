...
namespace Lite.BevTreeEditor
{
	public class BehaviourTreeEditor : EditorWindow
	{
		...
		private void SetupBTDebugging()
		{
			...
			PawnController ctrl = actor.GetController() as PawnController;
			if (ctrl == null)
				return;

			BTDebugHelper.DebugContext = ctrl.Context;

			BTDebugHelper.ClearHotTree();

			for (int i = 0; i < ctrl.GetTreeCount(); ++i)
				BTDebugHelper.AddHotTree(ctrl.GetTree(i));*/
