...
namespace Lite.BevTreeEditor
{
	public static class BTEditorUtils
	{
		...
		private const int BEZIER_H_OFFSET_VERTICAL = 100;
		...
		public static void DrawBezierVertical(Vector2 a, Vector2 b, Color color)
		{
			Vector2 start = a.y <= b.y ? a : b;
			Vector2 end = a.y <= b.y ? b : a;

			float vertDistance = Mathf.Abs(start.y - end.y);
			float horzDistance = Mathf.Abs(start.x - end.x);
			float lerp = Mathf.Min(Mathf.Clamp01(vertDistance / MIN_V_DISTANCE), Mathf.Clamp01(horzDistance / MIN_H_DISTANCE));
			float offset = Mathf.Lerp(0.0f, BEZIER_H_OFFSET_VERTICAL, lerp);

			Vector3 startTangent = new Vector3(start.x, start.y + offset, 0);
			Vector3 endTangent = new Vector3(end.x, end.y - offset, 0);
