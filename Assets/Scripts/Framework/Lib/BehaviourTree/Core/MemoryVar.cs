...
using Lite.BevTree.Serialization;
...
namespace Lite.BevTree
{
	public class MemoryVar : IJsonSerializable
	{
		...
		public void Change(BevBlackboard blackboard, object value)
		{
		...
		public T Evaluate<T>(BevBlackboard blackboard, T defValue)
		{
			if(!string.IsNullOrEmpty(m_valueAsVariableName))
			{
		...
		public bool HasValue<T>(BevBlackboard blackboard)
		{
			return false;//blackboard.HasItem<T>(m_valueAsVariableName);
		}
